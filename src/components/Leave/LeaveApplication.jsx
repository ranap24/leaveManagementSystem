...
function LeaveApplication({ method, loaderData }) {
  ...
  const calculateTotalDays = () => {
    ...
    if (
      ...
      const diffInDays =
        Math.round((diffInMs / (1000 * 3600 * 24)) * 10) / 10 + 1;
    ...
    <div className={classes.formContainer}>
      <Form method={method} className={classes.form}>
        ...
        <Input
          ...
          defaultValue={
            loaderData && loaderData.phoneNumber && loaderData.phoneNumber
          }
        />
        ...
        <Input
          ...
          defaultValue={
            loaderData && loaderData.fromDate && loaderData.fromDate
          }
        />
        {formError && formError.fromDate && (
          <p className={classes.error}>{formError.fromDate}</p>
        )}
        ...
        />
        {formError && formError.toDate && (
          <p className={classes.error}>{formError.toDate}</p>
        )}
        ...
        />
        {formError && formError.reason && (
          <p className={classes.error}>{formError.reason}</p>
        )}

        <button type="submit" className={classes.submitButton}>
          {method === "PUT" ? "Update" : "Apply"}
        </button>
        <button
          type="button"
